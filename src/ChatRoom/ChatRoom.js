...
export default function ChatRoom() { // hooks
  ...
  useEffect(() => {
    db.collection("chatRooms")
      .where("users", "array-contains", currentUser.uid) // user, all chats of current user, 
  ...
  const handleInput = (ev) => { //when serch we
    ev.preventDefault(); //when submmit prevent browser refresh
    setFilteredUsers([]); //clear search
  ...
  const createNewChatRoom = (receiver) => { 
    ...
    if (isCreated) {
      ...
      setSearchInput(""); //zanulqva stoinosta na inputa
      setFilteredUsers([]);
    } else {
      let id = v4(); //random 
      db.collection("chatRooms")
